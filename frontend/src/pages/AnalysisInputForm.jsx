...
import "../styles/AnalysisInputForm.css";
...
export default function AnalysisInputForm() {
  ...
  return (
    <div className="input-form-container">
      <div className={`blur-overlay ${showModal ? "active" : ""}`}></div>
      ...
      <div className="form-buttons">
        <button className="save-button" onClick={() => handleSubmit("save")}>임시 저장</button>
        <button
          className={isComplete ? "save-button" : "complete-button"}
          disabled={!isComplete}
      ...
      {showModal && (
        <div className="modal-box centered">
          <h3 className="modal-title">해시 검증</h3>
          ...
          <div className="form-buttons" style={{ justifyContent: 'center', marginTop: '20px' }}>
            <button className="save-button" onClick={handleUploadAndSubmit}>제출</button>
          </div>
        </div>
      )}
    </div>
